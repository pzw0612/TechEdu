...
//		BaseLines lines = genBaseLine(boundary,edgeList);
	...
	public static String outLineCellHandle(List<String> list){
		...
		Map<Integer, List<Integer>>  boundaryMap = boundaryCell2(list);
		List<Integer> lngList = new ArrayList<Integer>(boundaryMap.keySet());
		Collections.sort(lngList,Utils.integerSort);
		List<Integer>  lineboundaryList = null;
		OutLine outLine = new OutLine();
		for(Integer key: lngList){
...
//	}
	 ...
	 * @return
	 */
	static Map<Integer, List<Integer>>  boundaryCell2(List<String> list){
		Map<Integer, List<Integer>>  result = new HashMap<Integer, List<Integer>>();
		...
		for(String point: list){
			...
			lng = new Integer(arrStr[0]);
			lat = new Integer(arrStr[1]);
			tmpList = result.get(new Integer(lng));
			if(tmpList ==null){
				tmpList = new ArrayList<Integer>();
				result.put(new Integer(lng), tmpList);
			}
			tmpList.add(lat);
		}
...
//		System.out.println("outLine=" + outLine); 
		...
		List<Point> rawData = Utils.string2LngLat("121.70164,31.051872,121.520366,30.858731,121.878795,30.856373,121.855449,31.005978,121.781291,31.070694");
		Set<String> lngLatSet = split(rawData);
		List<String> lngLatList = new ArrayList<String>(lngLatSet);
		Collections.sort(lngLatList, Utils.stringSort);
		System.out.println("split=");
		System.out.println(lngLatList); 
